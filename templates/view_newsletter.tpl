<div class="display newsletters">
	<div class="floaticon">
		...
		{if $gContent->isOwner() || $gBitUser->hasPermission( 'p_newsletters_admin' )}
			<a href="{$smarty.const.NEWSLETTERS_PKG_URL}newsletters.php?nl_id={$gContent->mNewsletterId}">{biticon ipackage="icons" iname="edit" iexplain="edit"}</a>
			<a href="{$smarty.const.NEWSLETTERS_PKG_URL}newsletters.php?nl_id={$gContent->mNewsletterId}&amp;remove=1">{biticon ipackage="icons" iname="user-trash" iexplain="delete"}</a>
		{/if}
		...
		{/if}
		<a href="{$smarty.const.NEWSLETTERS_PKG_URL}send_newsletter.php?post_id={$post_id}">{biticon ipackage="icons" iname="internet-mail"  iexplain="email this post"}</a>
	</div>
